...
struct Cores {
	...
	void Diffuse(float red, float green, float blue) {
		this->RGB[0] = red;
		this->RGB[1] = green;
		this->RGB[2] = blue;
		this->RGB[3] = 1;
	...
	void Specular(float red, float green, float blue) {
		this->RGB[0] = red;
		this->RGB[1] = green;
		this->RGB[2] = blue;
		this->RGB[3] = 1;
	...
	void Emissive(float red, float green, float blue) {
		this->RGB[0] = red;
		this->RGB[1] = green;
		this->RGB[2] = blue;
		this->RGB[3] = 1;
...
struct Transfs_por_Fig {
	...
	Transfs_por_Fig(std::vector<Transformacao> transfs_aux, std::vector<Cores> cores_aux, std::string nick) {
		this->transfs = transfs_aux;
		this->cores = cores_aux;
...
struct FiguraData {
	...
	void Add_Pts_to_FiguraData(std::string nick, std::vector<Ponto> pts_aux) {
	...
	while (g) {
		...
		if (models) {
			...
			while (model != nullptr) {
				...
				if (colors) {
					...
					while (color != nullptr) {
						...
						if (nick == "ambient") {
							...
							float sh = color->FloatAttribute("value");
				...
				trfs_por_fig.push_back(Transfs_por_Fig(transfs, vect_cores, model->Attribute("file")));
